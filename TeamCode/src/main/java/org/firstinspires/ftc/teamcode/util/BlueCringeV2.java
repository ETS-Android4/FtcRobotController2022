...
@Config
@Autonomous(name = "BlueCringeV2")
public class BlueCringeV2 extends LinearOpMode {
    OpenCvWebcam webcam;
    CVPipeline pipeline;
...
//        bl.setPosition(0.02);
        ...
        Trajectory traj1 = tankDrive.trajectoryBuilder(startingPosition).forward(18).build();
        Trajectory traj2 = tankDrive.trajectoryBuilder(traj1.end().plus(new Pose2d(0, 0, Math.toRadians(90)))).forward(60).build();

        waitForStart();
        if (opModeIsActive()) {
            tankDrive.followTrajectory(traj1);
            switchFromTankToMec();
            sleep(300);
            mecanumDrive.turn(Math.toRadians(90));
            switchFromMecToTank();
            sleep(300);
            tankDrive.followTrajectory(traj2);
//            fleft.setPower(-0.1);
//            fright.setPower(-0.1);
//            bleft.setPower(-0.1);
//            bright.setPower(-0.1);
